...
describe('the species registry', () => {
  ...
  it('has shipped the base 24 plus five collections across two kits', () => {
    ...
    // 105 SINCE 4 AUG. 100 of it is WOODLAND rebuilt on the parts route —
    ...
    expect(shippedIn('birds')).toHaveLength(5)        // 18 rostered; five passerines, see below
  })
  ...
  it('leaves 236 species rostered but unshipped, on purpose', () => {
    ...
    expect(rostered.filter(id => !speciesRecord(id))).toHaveLength(215)
  })
