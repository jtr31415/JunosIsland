 ...
 * (`NAME_BANDS`, types.ts:474). Every band is a subrange of the generator's own
 ...
 * or regenerated by this module, and `Pet.id` embeds the name (`flow.ts:329`),
 * so touching one would break an id too. THIS IS WHY THE HATCH SWITCH DID NOT
 * RENAME ANYBODY: a pet already in a save keeps the name it was born with,
 * because `save.ts:301` reads `pets` back verbatim and every display path
 * (`album.ts:315,704,744`, `interactions.ts:202`) reads `pet.name` off the
 * record rather than deriving it from the species.
 *
 * ## THE HATCH IS WIRED. That one-line change has been made.
 *
 * This header used to end by saying the live hatch still called
 * `petName(defaultRng)` — unseeded — and that swapping it to `givenName()` was
 * a one-line change waiting on the audit. **It is no longer waiting.** On
 * 2 August 2026 the unseeded draw handed a six-year-old a rabbit called
 * *Defuck*, and Joe ruled: *"no dont change the generator, we hard fix the
 * first 24 animals instead"*. The hard fix for an animal IS its frozen name,
 * so `src/island/main.ts` now names a hatching friend with `givenName(species)`
 * — see the comment at that call site for the measured numbers.
 *
 * What that changes for you, in order of how likely it is to bite:
 *   - **A name is now per SPECIES, not per PET.** Two pets of one species carry
 *     one name. The deck deals without replacement for the first 24 pets, so
 *     this cannot happen until a child owns 25; from there it is the steady
 *     state. `Pet.id` stays unique regardless — it is `pet<N>-<name>` off an
 *     ever-growing index, and no release path exists to rewind that index.
 *   - **`tests/island/name-screen.test.ts` is now a shipping gate**, not a
 *     nicety. It screens all 320 allocated names against Joe's banned roots.
 *     If it goes red, a child is one hatch away from reading the thing it
 *     caught. Fix the name with a PIN; never by filtering the stream.
 *   - The audit is still Joe's and still outstanding. The brief's order was
 *     generate, audit, then ship — this shipped ahead of the audit because a
 *     live slur outranks a tidy sequence, and because the screen makes the
 *     narrow, urgent half of that audit mechanical.
 */
