 ...
 *
 *   - `animal-crocodile` — a sprawled, long-jawed reptile. The quadruped kit
 *     stands its legs under the body and gives it a cube skull; a crocodile
 *     expressed through it is a lizard-shaped dog. Wants `bespoke`.
 ...
 *
 * Africa therefore ships PARTIAL, thirteen of sixteen, and
 * `tests/island/species-africa.test.ts` asserts the three absences by name so
 * nobody half-fills the collection later by improvising a shape for them. That
 * improvisation is the exact failure roster §1's "kits before species" rules out.
 *
