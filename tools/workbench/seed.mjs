...
const TASKS = [
  {
    ...
    detail: 'Red-pen every lesson in the editor, status draft → vetted. Includes the school-verb check: does their school say "borrow" or "take" for the carry? L-ADD-4 and L-SUB-4 both use "borrow".',
  ...
  },
  {
    ...
    detail: 'Every close-spelled trap pair must be distinct in THEIR accent, not in a dictionary. Blocks Run D trap rungs. File arrives with Run D.',
...
const BACKLOG = [
  ...
  ['PB-002', 'Does v1 ship without adaptive difficulty?', "Fable's view: yes, since today's fixed difficulty is the v0 behaviour they already play. Joe's call.", 'open', ''],
  ...
  ['PB-004', 'A camera gesture is live that was never ruled on', 'Tapping their own land turns the island about that tile. Costless, cannot start a round, but it is a product change to a gesture Joe had explicitly ruled on. Veto is one line in interactions.ts (the focusOn call in the tile case).', 'open', ''],
  ['PB-005', 'Lighting review, tilt-shift, more fog, visual options menu (#43)', 'END OF PHASE 4. What is missing is measured fps on the actual mid-range Android tablet — nothing in this project has ever been measured there. Order: measure → amend the brief → build. ?flat already exists reserved for this. The DEFAULTS matter more than the options, because they will never open a settings menu.', 'open', ''],
  ['PB-006', 'Get it in front of Juno again (#34)', 'The highest-value action that is not code. Everything they reported on 27 July is fixed. It is also the only way to judge what was decided on a DPR-1 desktop at night by one person: the font, the prop shadows, the tap target, the 3:1 ratio, the camera gesture.', 'open', ''],
  ...
  ['PB-012', 'Pure play elements as a session reward (#39)', 'PARKED — not to be built without Joe. The observation matters as much as the idea: after an hour of earning, the reward they chose for themselves was unstructured looking. The cheapest "pure play element" may be to stop obstructing the play they have already invented.', 'parked', ''],
  ...
  ['PB-022', 'Biome & tile ladder (brief item 14)', 'docs/rock-hexes-proposal.md is a down-payment, and carries two questions: what a rock tile is FOR (if not habitable, choosing it silently slows their pet progress, which a six-year-old cannot weigh), and that "pure grey" is not available — bare mountain variants sample the atlas rock swatch, which Summer renders TAN.', 'open', ''],
  ...
  ['PB-026', 'Enclosed ponds are no longer constructible', 'A gameplay change Joe should veto if it is wrong. Confining water to the 19 drawable neighbourhoods is what lets the water cell carry its whole beach — but water now grows as coastline and never as a hole in the middle of their island.', 'open', ''],
